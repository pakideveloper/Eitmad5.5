                            ...
                            <div class="col-sm-12">
                                <div class="card-box">
                                    ...
                                    <div class="table-rep-plugin">
                                        <h4 class="m-t-0 header-title"><b>All industries</b></h4>
                                        <div class="table-responsive" data-pattern="priority-columns">
                                            <table id="tech-companies-1" class="table  table-striped">
                                                <thead>
                                                    <tr>
                                                        <th>Id</th>
                                                        <th data-priority="1">Industry Name</th>
                                                ...
                                                <tbody>
                                                   @foreach($industry as $Vindustry)
                                                    <tr>
                                                        
                                                        <td>{{$Vindustry->id}}</td>
                                                        <th>{{$Vindustry->industry_name}}</th>
                                                        <td><?php $monthNum  = Carbon\Carbon::parse($Vindustry->created_at)->format('m');
                                                $dateObj   = DateTime::createFromFormat('!m', $monthNum);
                                                $monthName = $dateObj->format('F'); 
                                                echo $monthName; // March?>
                                                {{ Carbon\Carbon::parse($Vindustry->created_at)->format('d,Y') }}</td>
                                                        <td><?php $monthNum  = Carbon\Carbon::parse($Vindustry->created_at)->format('m');
                                                $dateObj   = DateTime::createFromFormat('!m', $monthNum);
                                                $monthName = $dateObj->format('F'); 
                                                echo $monthName; // March?>
                                                {{ Carbon\Carbon::parse($Vindustry->updated_at)->format('d,Y') }}</td> 
                                                        <td>
                                                            ...
                                                            <form id="delete-form{{$Vindustry->id}}" 
                                                                ...
                                                                <i  onclick="return deleteCategory({{$Vindustry->id}});" class="fa fa-trash-o" style="cursor: pointer;     margin-left: 10px;     color: red;"></i>
                                                            </form>
                                                        </td>                     
                                                    </tr>
                                                     @endforeach
